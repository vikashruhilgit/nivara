...
|---|-------|---------------------------|---------------------------|--------|--------|
| 1 | Backend benchmark service (per-market + blended) | AC #1, #2, #3 | 1 modify (backend/app/analysis/ or backend/app/intelligence/ — add benchmark module), 2 create (backend/app/intelligence/benchmark.py, backend/tests/test_benchmark.py) | pandas, Yahoo Finance data for ^NSEI/^GSPC | LAUNCHABLE |
